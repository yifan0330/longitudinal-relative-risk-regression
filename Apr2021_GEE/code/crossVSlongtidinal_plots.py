...
def _plot_voxel(voxel_id, title, voxel_ids, intercept, age, ageDiff, ageBYageDiff):
    ...
    plt.figure()
    plt.plot(
        age_test + 63.1, np.exp(get(intercept) + get(age) * age_test), linestyle="--"
    )
    age_segments_diff_plot = np.r_[
        np.arange(50, 56), np.arange(60, 66), np.arange(70, 76)
    ]
    age_segments_diff_est = np.tile(np.arange(0, 6), 3)
    age_segments_age = (
        np.r_[np.repeat(50, 6), np.repeat(60, 6), np.repeat(70, 6)] - 63.1
    )
    prob_age_diff = np.exp(
        get(intercept)
        + get(age) * age_segments_age
        + get(ageDiff) * age_segments_diff_est
    )
    prob_inter = np.exp(
        get(intercept)
        + get(age) * age_segments_age
        + get(ageDiff) * age_segments_diff_est
        + get(ageBYageDiff) * age_segments_age * age_segments_diff_est
    )
...
def main():
    ...
    intercept = _load(
        GEEDIR / "results_July_pgee_interaction" / f"estimate_{names[0]}_GEE.nii.gz"
    )
    age = _load(
        GEEDIR / "results_July_pgee_interaction" / f"estimate_{names[1]}_GEE.nii.gz"
    )
    ageDiff = _load(
        GEEDIR / "results_July_pgee_interaction" / f"estimate_{names[2]}_GEE.nii.gz"
    )
    ageBYageDiff = _load(
        GEEDIR / "results_July_pgee_interaction" / f"estimate_{names[5]}_GEE.nii.gz"
    )
    for voxel, title in [
        (442745, "Deep WM, voxel 30, 70, 45"),
        (394150, "Deep WM, voxel 29, 81, 40"),
        (491255, "Periventricular WM, voxel 37, 58, 50"),
        (433016, "Periventricular WM, voxel 38, 72, 44"),
    ]:
