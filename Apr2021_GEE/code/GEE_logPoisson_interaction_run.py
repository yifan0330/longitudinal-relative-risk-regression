...
def _read_rdata(path: Path) -> dict:
    ...
    try:
        ...
        raise RuntimeError(f'Cannot read RData file {path}; install rdata or pyreadr') from exc
...
def _load_lesions(cleaned: bool = True):
    name = 'lesions_atleast6_cleaned_Apr2021.RData' if cleaned else 'lesions_atleast6_Apr2021.RData'
...
def _prepare_prior_visits() -> tuple[pd.DataFrame, np.ndarray]:
    subjects = pd.read_csv('/well/nichols/users/kindalov/FMRIB/bianca_1vis_2vis_overlap_filenames_Apr2021.txt', header=None, sep=r'\s+', engine='python')
...
def _panel_for_voxel(i: int, lesions_vis1: np.ndarray, lesions_vis2: np.ndarray, subset_idx: np.ndarray, ids: np.ndarray, df_visits: pd.DataFrame, cleaned: bool):
    n_subj = lesions_vis1.shape[1]
    voxel = subset_idx[i]
    panel = pd.DataFrame({'y': np.r_[lesions_vis1[voxel, :], lesions_vis2[voxel, :]],
...
CLEANED = True
SUBSET_SIZE = 1000
OUTPUT_DIR = GEEDIR / 'temp_July_gee_interaction'
FUNCTION_FILE = Path(__file__).with_name('gee_logPoisson_dispersion_fn.py')
FUNCTION_NAME = 'gee_dispersion_run'
INTERACTION = True
PENALTY = False
...
def _init_worker(subset_idx, ids, df_visits, lesions_vis1, lesions_vis2):
    _WORKER.update(subset_idx=subset_idx, ids=ids, df_visits=df_visits, lesions_vis1=lesions_vis1, lesions_vis2=lesions_vis2)


def _fit_one(i):
    try:
        panel, voxel = _panel_for_voxel(i, _WORKER['lesions_vis1'], _WORKER['lesions_vis2'], _WORKER['subset_idx'], _WORKER['ids'], _WORKER['df_visits'], CLEANED)
        n_visits = panel['vis'].nunique()
        n_subj = len(panel) // n_visits
        if CLEANED:
            cols = [np.ones(len(panel)), panel['age_vis2'], panel['age_diff'], panel['sexM'], panel['headsize']]
            if INTERACTION:
                cols.extend([panel['age_vis2'] * panel['age_diff'], panel['age_vis2'] * panel['sexM']])
            else:
                cols.append(panel['age_vis2'] * panel['sexM'])
        else:
            cols = [np.ones(len(panel)), panel['avg_age'], panel['age_diff_vis2'], panel['sexM']]
        X = np.column_stack([np.asarray(c, dtype=float) for c in cols])
        y = panel['y'].to_numpy(dtype=float)
        if USE_STATS_GEE_LOGISTIC:
            import statsmodels.api as sm
            model = sm.GEE(y, X, groups=np.repeat(np.arange(n_subj), n_visits),
            ...
            out = {'beta': result.params, 'beta_se_sandwich': result.bse,
                   'alpha': float(dep[0]) if dep.size else np.nan,
                   'phi': float(result.scale), 'iterations': 0, 'voxel': i + 1}
            if INTERACTION:
                out['beta_se_sandwich_modified'] = result.bse
        else:
            fn = _load_function(FUNCTION_FILE, FUNCTION_NAME)
            model = fn(y=y, X=X, n_subj=n_subj, n_visits=n_visits, covariance='Exchangeable', tol=TOL, max_iter=MAX_ITER)
            if isinstance(model, float) and np.isnan(model):
                return np.nan
            out = {'beta': np.asarray(model['beta']), 'beta_se_model': model['beta_se_model'],
                   'beta_se_model_trace': model['beta_se_model_trace'], 'beta_se_sandwich': model['beta_se_sandwich'],
                   'alpha': model['alpha'], 'phi': model['phi'], 'iterations': model['iterations'], 'voxel': i + 1}
...
def main(argv=None):
    parser = argparse.ArgumentParser(description='Run voxelwise GEE model for one subset.')
    ...
    if args.n_cores > 1:
        ...
        with ProcessPoolExecutor(max_workers=args.n_cores, initializer=_init_worker, initargs=(subset_idx, ids, df_visits, lesions_vis1, lesions_vis2)) as ex:
