...
def _read_pickle_or_rdata(path: Path) -> dict:
    ...
    try:
        ...
        raise RuntimeError(
            f"Cannot read {path}; install rdata/pyreadr or use Python-generated pickle output"
        ) from exc
...
def _save_like(data, like_img, stem: Path):
    path = Path(
        str(stem) if str(stem).endswith((".nii", ".nii.gz")) else str(stem) + ".nii.gz"
    )
...
def main():
    ...
    voxel_ids = (
        pd.read_csv(
            TEMPDIR / "voxel_IDs_atleast6_cleaned_Apr2021.dat",
            header=None,
            sep=r"\s+",
            engine="python",
        )
    ...
    for j in range(1, n_subsets + 1):
        ...
        stop = (
            min(len(voxel_ids), len(lesions_vis1))
            if j == n_subsets
            else min(subset_size * j, len(voxel_ids))
        )
        subset_idx = np.arange(start, stop)
        data = _read_pickle_or_rdata(
            GEEDIR / "temp_July_gee_interaction" / f"GEE_subset_{j}.RData"
        )
    ...
    for i, name in enumerate(names_covs):
        _save_like(
            _set_voxels(image_template, voxel_ids, estimates[:, i]),
            brain_img,
            outdir / f"estimate_{name}_GEE",
        )
        ...
        _save_like(
            _set_voxels(image_template, voxel_ids, zscores[:, i]),
            brain_img,
            outdir / f"zscore_{name}_GEE",
        )
        print("-----")
    ...
    with (outdir / "results_interaction_GEE_2635subjs.Rdata").open("wb") as f:
        pickle.dump(
            {k: v for k, v in locals().items() if k not in {"brain_img"}},
            f,
    ...
    for label, arr in [
        ("RR_age", np.exp(coef_age)),
        ("RR_ageDiff", np.exp(coef_ageDiff)),
        ("RR_ageBYageDiff", np.exp(coef_age + coef_ageDiff + coef_ageBYageDiff)),
    ]:
        _save_like(_mask_outside(arr, voxel_ids), brain_img, outdir / label)
    plt.figure()
    plt.plot(
        np.exp(coef_age).ravel(order="F")[voxel_ids - 1],
        np.exp(coef_ageDiff).ravel(order="F")[voxel_ids - 1],
        ".",
    )
    plt.figure()
    plt.plot(
        np.exp(coef_age).ravel(order="F")[voxel_ids - 1],
        np.exp(coef_age + coef_ageDiff + coef_ageBYageDiff).ravel(order="F")[
            voxel_ids - 1
        ],
        ".",
    )
