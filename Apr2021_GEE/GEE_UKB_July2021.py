...
def main() -> None:
    ...
    se_ratios_gee, idx_temp_gee, _ = analyze_se_ratios(
        gee_path, len(NAMES_COVS), 8, NAMES_COVS, "GEE"
    )
    ...
    for i, name in enumerate(NAMES_COVS):
        ...
        print_summary(
            f"GEE non-separated SE ratio: {name}",
            se_ratios_gee[i, keep_gee],
            [0, 0.5, 1],
        )

    se_ratios_pgee, idx_temp_pgee, _ = analyze_se_ratios(
        pgee_path, len(NAMES_COVS), 9, NAMES_COVS, "PGEE"
    )
    ...
    for i, name in enumerate(NAMES_COVS):
        ...
        print_summary(
            f"PGEE non-separated SE ratio: {name}",
            se_ratios_pgee[i, keep_pgee],
            [0, 0.5, 1],
        )
