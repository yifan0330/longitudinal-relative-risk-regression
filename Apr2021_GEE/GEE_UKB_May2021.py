...
NAMES_COVS_PLOTS = ['Intercept', 'Age (visit 1)', 'Time difference', 'Sex', 'Head size', 'Age (visit 1):Sex']
...
def read_payload(path: Path) -> dict[str, Any]:
    ...
    try:
        ...
        raise RuntimeError(f'Cannot read {path}; install rdata or pyreadr, or provide a Python pickle payload') from exc
...
def read_voxel_ids(path: Path = TEMPDIR / 'voxel_IDs_atleast6_cleaned_Apr2021.dat') -> np.ndarray:
    return pd.read_csv(path, header=None, sep=r'\s+', engine='python').to_numpy().reshape(-1).astype(int)
...
def print_summary(label: str, values: Iterable[float], quantiles: Iterable[float] = QUANTS_FULL) -> dict[str, Any]:
...
def _is_missing_output(obj: Any) -> bool:
    return obj is None or (isinstance(obj, float) and np.isnan(obj)) or (isinstance(obj, dict) and 'error' in obj)
...
def se_ratio_last_iter(output_all: list[Any], p: int, expected_len: int) -> np.ndarray:
    ...
    for out in output_all:
        if _is_missing_output(out) or (_len_output(out) not in {expected_len, expected_len - 1, expected_len + 1} and not isinstance(out, dict)):
...
def extract_iterations(output_all: list[Any], expected_len: int) -> np.ndarray:
    ...
    for out in output_all:
        if _is_missing_output(out):
            ...
            continue
        try:
            vals.append(float(np.asarray(_field(out, 7, ('iterations',))).reshape(-1)[0]))
        except Exception:
            vals.append(np.nan)
...
def extract_model_se(output_all: list[Any], p: int, covariate_index: int) -> np.ndarray:
    ...
    for out in output_all:
        ...
        try:
            ...
            try:
                trace = matrix_by_cols(_field(out, 3, ('beta_se_model_trace', 'se_model_trace')), p)
...
def plot_se_histogram(se_ratios_gee: np.ndarray, se_ratios_pgee: np.ndarray, out_path: Path) -> None:
    dat = pd.DataFrame({
        'x': np.r_[se_ratios_gee[3, :], se_ratios_pgee[3, :]],
        'group': ['GEE'] * se_ratios_gee.shape[1] + ['PGEE'] * se_ratios_pgee.shape[1],
    })
    grid = sns.FacetGrid(dat, col='group', height=5, aspect=1.4, sharex=True, sharey=False)
...
def plot_scatter(x: np.ndarray, y: np.ndarray, out_path: Path, xlabel: str, ylabel: str, limits: tuple[float, float], smooth: bool = False) -> None:
    ...
    if smooth:
        try:
            ...
            pass
    lo, hi = limits
    ax.plot([lo, hi], [lo, hi], linestyle='--', color='red' if xlabel.startswith('p') else 'black', linewidth=0.7)
    ax.set_xlim(lo, hi)
    ax.set_ylim(lo, hi)
...
def p_adjust_bh(pvals: np.ndarray) -> np.ndarray:
    ...
    order = np.argsort(pv)
    ranked = pv[order]
    adjusted = np.minimum.accumulate((ranked.size / np.arange(ranked.size, 0, -1)) * ranked[::-1])[::-1]
...
def zscores_plot(slice_num: int, mni152: np.ndarray, img: np.ndarray, mask: np.ndarray, voxel_ids: np.ndarray,
                 fdr: bool, fdr_threshold: float, name: Path, legend: bool, vmin: float, vmax: float,
    ...
    im = ax.imshow(overlay, cmap=cmap, vmin=vmin, vmax=vmax, alpha=0.85, interpolation='nearest')
    ax.set_axis_off()
    if title:
        ax.text(0.03, 0.97, title, color='white', fontsize=18, transform=ax.transAxes, va='top')
    if legend:
        fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
    _save_figure(Path(f'{name}{slice_num}.pdf'))


def analyze_se_ratios(result_path: Path, p: int, expected_len: int, names_covs: list[str], prefix: str) -> tuple[np.ndarray, np.ndarray, np.ndarray]:
    ...
    print(f'{prefix} all covariates > 10:', np.where(np.logical_and.reduce(covs))[0] + 1)
...
def sex_estimate_summaries(results_dir: Path, names_covs: list[str], voxel_ids: np.ndarray, exclude_idx: np.ndarray) -> None:
    _, sex_est = read_nifti(results_dir / f'estimate_{names_covs[3]}_GEE.nii.gz')
    print_summary('sexM estimate: all voxels', flat_values(sex_est, voxel_ids), [0, 0.25, 0.5, 0.75, 0.99, 1])
    keep = np.ones(len(voxel_ids), dtype=bool)
    keep[exclude_idx] = False
    print_summary('sexM estimate: non-separated voxels', flat_values(sex_est, voxel_ids[keep]), [0, 0.25, 0.5, 0.75, 0.99, 1])


def compare_zscore_plots(gee_dir: Path, pgee_dir: Path, plot_dir: Path, names_covs: list[str], voxel_ids: np.ndarray,
                         idx_temp_gee: np.ndarray, output_all_gee: list[Any] | None = None,
                         include_model_variance: bool = False) -> None:
    ...
    plot_scatter(z_sex_gee[idx_temp_gee], z_sex_pgee[idx_temp_gee], plot_dir / 'zscores_sex_sep_scatter.pdf', 'GEE Sex z-scores', 'PGEE Sex z-scores', (-20, 20))
    keep = np.ones(len(voxel_ids), dtype=bool)
    keep[idx_temp_gee] = False
    plot_scatter(z_sex_gee[keep], z_sex_pgee[keep], plot_dir / 'zscores_sex_nonsep_scatter.pdf', 'GEE Sex z-scores', 'PGEE Sex z-scores', (-20, 20))
    if include_model_variance and output_all_gee is not None:
        ...
        plot_scatter(z_model[idx_temp_gee], z_sex_pgee[idx_temp_gee], plot_dir / 'zscores_sex_sep_modelVar_scatter.pdf', 'GEE Sex z-scores', 'PGEE Sex z-scores', (-20, 20))
        plot_scatter(z_model[keep], z_sex_pgee[keep], plot_dir / 'zscores_sex_nonsep_modelVar_scatter.pdf', 'GEE Sex z-scores', 'PGEE Sex z-scores', (-20, 20))


def zscore_image_series(results_dir: Path, names_covs: list[str], names_covs_plots: list[str], voxel_ids: np.ndarray,
                        brain_mask: np.ndarray, mni152: np.ndarray, fdr: bool = False) -> None:
    p = len(names_covs)
    for i, name in enumerate(names_covs):
        ...
        zscores_plot(45, mni152, z_temp, brain_mask, voxel_ids, fdr, 0.05,
                     results_dir / 'plots' / f'{name}_zscores_{"fdr" if fdr else "nofdr"}_z', i == p - 1, -5, 5,
                     names_covs_plots[i])


def scalar_image_series(results_dir: Path, image_name: str, voxel_ids: np.ndarray, brain_mask: np.ndarray, mni152: np.ndarray,
                        vmin: float, vmax: float, title: str, cmap: str = 'RdBu_r') -> None:
    _, img = read_nifti(results_dir / f'{image_name}_GEE.nii.gz')
    for slice_num, legend in [(40, False), (45, False), (50, True)]:
        zscores_plot(slice_num, mni152, img, brain_mask, voxel_ids, False, 0.05,
                     results_dir / 'plots' / f'{image_name}_z', legend, vmin, vmax, title if slice_num == 40 else '', cmap)


def empirical_probability_plots(voxel_ids: np.ndarray, brain_mask: np.ndarray, mni152: np.ndarray, plot_dir: Path) -> np.ndarray:
    _, empir_prob_vis1 = read_nifti(IMAGEDIR_VIS1 / 'Apr2021_cleaned_empir_prob_mask.nii.gz')
    _, empir_prob_vis2 = read_nifti(IMAGEDIR_VIS2 / 'Apr2021_cleaned_empir_prob_mask.nii.gz')
    for label, img, title in [('empir_vis1', empir_prob_vis1, 'sqrt(p1)'), ('empir_vis2', empir_prob_vis2, 'sqrt(p2)')]:
        for slice_num, legend in [(40, False), (45, False), (50, True)]:
            zscores_plot(slice_num, mni152, np.sqrt(img), brain_mask, voxel_ids, False, 0.05,
                         plot_dir / f'{label}_z', legend, 0, 0.7, title if slice_num == 40 else '', 'viridis')
    rr = empir_prob_vis2 / empir_prob_vis1
    finite_ids = voxel_ids[np.isfinite(flat_values(rr, voxel_ids))]
    for slice_num, legend in [(40, False), (45, False), (50, True)]:
        zscores_plot(slice_num, mni152, rr, brain_mask, finite_ids, False, 0.05,
                     plot_dir / 'empir_RR_z', legend, 0, 2, 'p2/p1' if slice_num == 40 else '', 'RdBu_r')
    return finite_ids
...
def empirical_scatterplots(voxel_ids: np.ndarray, plot_dir: Path) -> None:
    ...
    print('p2 > p1, p2 == p1, p2 < p1:', int(np.sum(y > x)), int(np.sum(y == x)), int(np.sum(y < x)))
...
def alpha_rr_plot(voxel_ids: np.ndarray, results_dir: Path, plot_dir: Path) -> None:
    ...
    try:
        sns.regplot(x=rr[keep], y=alpha[keep], lowess=True, scatter=False, color='red', ax=ax)
    except Exception:
        pass
    ax.set_xlim(0, 3)
    ax.set_ylim(-0.1, 1)
...
def separated_voxel_plot(voxel_ids: np.ndarray, idx_temp_gee: np.ndarray, brain_mask: np.ndarray, mni152: np.ndarray, plot_dir: Path) -> None:
    _, empir_prob_vis1 = read_nifti(IMAGEDIR_VIS1 / 'Apr2021_cleaned_empir_prob_mask.nii.gz')
    ...
    zscores_plot(50, mni152, img, brain_mask, voxel_ids, False, 0.05, plot_dir / 'sep_z', False, 0, 10, '', 'viridis')


def lesion_volume_scatter_and_gee(interaction_with_age_diff: bool, plot_dir: Path | None = None) -> Any:
    import statsmodels.api as sm
    df = pd.read_csv(TEMPDIR / 'df_visits_cleaned_Apr2021.dat', sep=r'\s+', engine='python')
    ...
    df = df[['eid_8107', 'age_vis2', 'age_vis3', 'sexM', 'headsize', 'X25781.2.0', 'X25781.3.0']].copy()
    ...
    panel = pd.DataFrame({'y': np.r_[y1, y2], 'vis': np.r_[np.ones(n_subj, dtype=int), np.full(n_subj, 2, dtype=int)], 'eid_8107': np.r_[ids, ids]})
    panel = panel.merge(df, on='eid_8107', how='left').sort_values(['eid_8107', 'vis']).reset_index(drop=True)
    panel['age_diff'] = panel['age_diff'].to_numpy() * np.tile([0, 1], n_subj)
    y = np.log(panel['y'].replace(0, np.nan).to_numpy(float))
    exog_parts = [np.ones(len(panel)), panel['age_vis2'], panel['age_diff'], panel['sexM'], panel['headsize'], panel['age_vis2'] * panel['sexM']]
    ...
    model = sm.GEE(y[keep], exog[keep, :], groups=panel.loc[keep, 'eid_8107'], family=sm.families.Gaussian(), cov_struct=sm.cov_struct.Exchangeable())
    result = model.fit()
    print(pd.DataFrame({'coef': result.params, 'std_err': result.bse, 'z': result.tvalues, 'p': result.pvalues}, index=names))
    return result
...
def main() -> None:
    ...
    se_ratios_gee, idx_temp_gee, _ = analyze_se_ratios(gee_results_dir / 'results_exch_GEE_2635subjs.Rdata', len(NAMES_COVS), 8, NAMES_COVS, 'GEE')
    sex_estimate_summaries(gee_results_dir, NAMES_COVS, voxel_ids, idx_temp_gee)
    for i, name in enumerate(NAMES_COVS):
        keep = np.ones(se_ratios_gee.shape[1], dtype=bool)
        keep[idx_temp_gee] = False
        print_summary(f'GEE non-separated SE ratio: {name}', se_ratios_gee[i, keep], [0, 0.5, 1])
    se_ratios_pgee, idx_temp_pgee, _ = analyze_se_ratios(pgee_results_dir / 'results_exch_GEE_2635subjs.Rdata', len(NAMES_COVS), 9, NAMES_COVS, 'PGEE')
    hat = extract_hat_indices(load_output_all(pgee_results_dir / 'results_exch_GEE_2635subjs.Rdata'))
    if hat is not None:
    ...
    compare_zscore_plots(gee_results_dir, pgee_results_dir, gee_results_dir / 'plots', NAMES_COVS, voxel_ids, idx_temp_gee, gee_output, include_model_variance=True)
    zscore_image_series(gee_results_dir, NAMES_COVS, NAMES_COVS_PLOTS, voxel_ids, brain_mask, mni152, fdr=True)
    zscore_image_series(pgee_results_dir, NAMES_COVS, NAMES_COVS_PLOTS, voxel_ids, brain_mask, mni152, fdr=True)
    ...
    finite_ids = empirical_probability_plots(voxel_ids, brain_mask, mni152, gee_results_dir / 'plots')
    empirical_scatterplots(finite_ids, gee_results_dir / 'plots')
    alpha_rr_plot(voxel_ids, pgee_results_dir, gee_results_dir / 'plots')
    separated_voxel_plot(voxel_ids, idx_temp_gee, brain_mask, mni152, gee_results_dir / 'plots')
    lesion_volume_scatter_and_gee(interaction_with_age_diff=False, plot_dir=gee_results_dir / 'plots')
