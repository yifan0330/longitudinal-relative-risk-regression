...
DEFAULT_REPLICATIONS = HERE / "results" / "full" / "replications.csv"
DEFAULT_OUTPUT_DIR = HERE / "results" / "full" / "pp_plots"
DEFAULT_TABLE4 = HERE / "results" / "full" / "table4" / "table4_variance_coverage.csv"
OR_METHODS = frozenset({"OR-GEE-ZY", "OR-PGEE-ZY"})
METHOD_ORDER = ("RR-GEE", "RR-PGEE", "OR-GEE-ZY", "OR-PGEE-ZY")
METHOD_LABELS = {
    "RR-GEE": "RR-GEE",
    "RR-PGEE": "RR-PGEE",
    "OR-GEE-ZY": "OR-GEE",
...
def bootstrap_pp_curve(
    ...
    *,
    ...
    """Return a bootstrap mean PP curve and pointwise percentile interval."""
    ...
    bootstrap_samples = np.sort(
        rng.choice(values, size=(n_bootstrap, values.size), replace=True),
        axis=1,
    )
    mean_p = bootstrap_samples.mean(axis=0)
    alpha = 1.0 - confidence_level
    lower_p, upper_p = np.quantile(
        bootstrap_samples,
        [alpha / 2.0, 1.0 - alpha / 2.0],
        axis=0,
    )
    expected_p = (np.arange(1, values.size + 1) - 0.5) / values.size
    return (
        -np.log10(expected_p),
        -np.log10(mean_p),
        -np.log10(upper_p),
...
def save_pp_plot(
    ...
    *,
    ...
    """Save one four-method bootstrap-mean PP-plot for a simulation scenario."""
    ...
    for method in METHOD_ORDER:
        ...
        method_seed = bootstrap_seed + zlib.crc32(f"{scenario}:{method}".encode())
        expected, mean_observed, lower, upper = bootstrap_pp_curve(
            method_values,
    ...
    axis_max = 1.02 * max(
        max(float(expected.max()), float(upper.max()))
        for expected, _, _, upper, _ in plot_lines
    )
    ...
    for expected, mean_observed, lower, upper, label in plot_lines:
        line = ax.plot(
            expected[::-1],
            mean_observed[::-1],
            linewidth=1.8,
