...
def main():
    ...
    changes_subjects = np.column_stack([np.sum((v1 == 0) & (v2 == 0), axis=0), np.sum((v1 == 0) & (v2 == 1), axis=0), np.sum((v1 == 1) & (v2 == 0), axis=0), np.sum((v1 == 1) & (v2 == 1), axis=0)])
    changes_subjects = np.column_stack([changes_subjects, (changes_subjects[:,2]+changes_subjects[:,3])/v1.shape[0], (changes_subjects[:,1]+changes_subjects[:,3])/v1.shape[0]])
    changes_voxels = np.column_stack([np.sum((v1 == 0) & (v2 == 0), axis=1), np.sum((v1 == 0) & (v2 == 1), axis=1), np.sum((v1 == 1) & (v2 == 0), axis=1), np.sum((v1 == 1) & (v2 == 1), axis=1)])
    changes_voxels = np.column_stack([changes_voxels, (changes_voxels[:,2]+changes_voxels[:,3])/v1.shape[1], (changes_voxels[:,1]+changes_voxels[:,3])/v1.shape[1]])
    print(pd.DataFrame(changes_subjects, columns=["00","01","10","11","prb_vis1","prb_vis2"]).describe())
    print(pd.DataFrame(changes_voxels, columns=["00","01","10","11","prb_vis1","prb_vis2"]).describe())
