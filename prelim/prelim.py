...
def build_cleaned_visits(ids):
    vis1 = read_table("/well/nichols/users/kindalov/FMRIB/Longitudinal/funpack/Vis0.tsv", sep="\t").rename(columns=lambda c: "eid_34077" if c == read_table("/well/nichols/users/kindalov/FMRIB/Longitudinal/funpack/Vis0.tsv", sep="\t").columns[0] else c)
    vis2 = read_table("/well/nichols/users/kindalov/FMRIB/Longitudinal/funpack/Vis2.tsv", sep="\t"); vis2 = vis2.rename(columns={vis2.columns[0]: "eid_34077"})
    vis3 = read_table("/well/nichols/users/kindalov/FMRIB/Longitudinal/funpack/Vis3.tsv", sep="\t"); vis3 = vis3.rename(columns={vis3.columns[0]: "eid_34077"})
    bridge = read_table("/well/nichols/projects/UKB/SMS/bridge_8107_34077.tsv", header=True); bridge.columns = ["eid_8107", "eid_34077"]
    ...
    df = vis1.merge(vis2, on=["eid_8107","eid_34077"]).merge(vis3, on=["eid_8107","eid_34077"])
    ...
    conditions = [1081,1082,1083,1086,1240,1243,1244,1245,1246,1247,1258,1259,1261,1262,1263,1264,1266,1408,1409,1410,1434,1491,1583,1626]
...
def main():
    ...
    subjects = read_table("/well/nichols/users/kindalov/FMRIB/bianca_1vis_2vis_overlap_filenames_Apr2021.txt", header=False).iloc[:,0].astype(str)
    ...
    write_table(pd.DataFrame(voxel_idx), Path(TEMPDIR) / "voxel_IDs_atleast6_cleaned_Apr2021.dat", header=False)
    lesions_vis1 = all_subj(df, IMAGEDIR_VIS1, voxel_idx)
    lesions_vis2 = all_subj(df, IMAGEDIR_VIS2, voxel_idx)
    save_rdata(Path(TEMPDIR) / "lesions_atleast6_cleaned_Apr2021.RData", lesions_vis1=lesions_vis1, lesions_vis2=lesions_vis2)
    print(pd.Series(empir2.ravel(order="F")[voxel_idx-1] - empir1.ravel(order="F")[voxel_idx-1]).describe())
