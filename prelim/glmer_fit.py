...
def fit_glm_fn(
    ...
    family = sm.families.Binomial(
        sm.families.links.Probit() if link_fn == "probit" else sm.families.links.Logit()
    )
    ...
    def fit_one(j):
        ...
        try:
            ...
            result = smf.glm(formula, family=family, data=current).fit(
                maxiter=10000 if GLMmethod == 2 else 100
            )
            return {
                "parameter": list(result.params.index),
                "zscore": result.tvalues.to_numpy(),
                "estimate": result.params.to_numpy(),
                "stderr": result.bse.to_numpy(),
                "converged_br": getattr(result, "converged", True),
                "status_br": True,
                "voxel": j + 1,
            }
    ...
    if outputdir is not None:
        name = (
            f"GLM_{method}_Nvars_{n_covs}_results.RData"
            if subset is None
            else f"GLM_subset_{subset}_{method}_Nvars_{n_covs}_results.RData"
        )
