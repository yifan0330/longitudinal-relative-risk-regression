    ...
    try:
        ...
        panel = pd.DataFrame(
            {
                ...
                "vis": np.r_[np.ones(n), np.full(n, 2)],
                "eid_8107": np.r_[ids, ids],
            }
        )
        panel = panel.merge(df_visits, on="eid_8107", how="left")
        panel["age_diff_vis2"] = (panel["age_vis2"] - panel["avg_age"]) * np.tile(
            [-1, 1], n
        )
        ...
        try:
            ...
            model = sm.BinomialBayesMixedGLM.from_formula(
                "y ~ age_diff_vis2", {"subject": "0 + C(eid_8107)"}, panel
            )
            res = model.fit_vb()
        ...
        return {
            ...
            "stderr": se,
            "zscore": est / se,
            "empir_vis1": r_linear_get(empir1, [vid])[0],
            "empir_vis2": r_linear_get(empir2, [vid])[0],
            "RaEvar": var,
            "voxel": i + 1,
        }
...
def main(argv=None):
    ...
    empir1, _ = load_nifti(Path(IMAGEDIR_VIS1) / "empir_prob_mask.nii.gz")
    ...
    voxel_ids = (
        read_table(Path(TEMPDIR) / "voxel_IDs_nonzero.dat", header=False)
        .to_numpy()
        .ravel()
    )
    ...
    output = (
        Pool(n_cores).map(fit_glmer, jobs)
        if n_cores > 1
