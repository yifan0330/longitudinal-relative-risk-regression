...
def main():
    ...
    voxel_idx = (
        np.flatnonzero(((empir1 + empir2) >= 6 / len(complete_df)) & (brain == 1)) + 1
    )
    write_table(
        pd.DataFrame(voxel_idx), Path(TEMPDIR) / "voxel_IDs_CVR.dat", header=False
    )
    lesions_vis1 = all_subj(complete_df, IMAGEDIR_VIS1, voxel_idx)
    lesions_vis2 = all_subj(complete_df, IMAGEDIR_VIS2, voxel_idx)
    save_rdata(
        Path(TEMPDIR) / "lesions_atleast6_CVR.RData",
