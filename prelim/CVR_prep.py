...
def smoking_score(df, visit):
    n = len(df)
    status = _col(df, f"X20116.{visit}.0").replace({-3: np.nan})
    start = _col(df, f"X3436.{visit}.0").where(lambda s: ~s.isin([-1, -3]))
    start = start.combine_first(
        _col(df, f"X2867.{visit}.0").where(lambda s: ~s.isin([-1, -3]))
    )
    stop = _col(df, f"X6194.{visit}.0").where(lambda s: ~s.isin([-1, -3]))
    stop = stop.combine_first(
        _col(df, f"X2897.{visit}.0").where(lambda s: ~s.isin([-1, -3]))
    )
    years = (stop - start).where(stop.notna(), df[f"age_vis{visit}"] - start)
    num = (
        _col(df, f"X6183.{visit}.0")
        .where(lambda s: s >= 0)
        .combine_first(_col(df, f"X2887.{visit}.0").where(lambda s: s >= 0))
    )
    stopped = (
        (_col(df, f"X2907.{visit}.0") == 1) | (_col(df, f"X3486.{visit}.0") == 2)
    ).astype(float)
    yearscomp = years.where(stopped == 0, years - 0.5)
    yearscomp = yearscomp.mask(years == 0, 0)
    pack = (num / 20) * yearscomp
    pack = (
        pack.mask(yearscomp == 0, 0)
        .mask((start <= 16) & (stop <= 16))
        .mask((start == stop) & (stopped == 1), 0)
    )
    pack = pack.mask(status == 0, 0)
    score = pd.Series(np.nan, index=df.index)
    score[pack < 10] = 0
    score[(pack >= 10) & (pack < 50)] = 1
    score[pack >= 50] = 2
    score[status == 0] = 0
    return score
...
def medication(df, visit):
    m = _col(df, f"X6177.{visit}.0")
    f = _col(df, f"X6153.{visit}.0")
    chol = pd.Series(0.0, index=df.index)
...
def bp_risk(df, visit, med_bp):
    da = (_col(df, f"X4079.{visit}.0") + _col(df, f"X4079.{visit}.1")) / 2
    dm = (_col(df, f"X94.{visit}.0") + _col(df, f"X94.{visit}.1")) / 2
    sa = (_col(df, f"X4080.{visit}.0") + _col(df, f"X4080.{visit}.1")) / 2
    sman = (_col(df, f"X93.{visit}.0") + _col(df, f"X93.{visit}.1")) / 2
    high = (da >= 90) | (dm >= 90) | (sa >= 140) | (sman >= 140)
    missing = da.isna() & dm.isna() & sa.isna() & sman.isna()
    bp = pd.Series(0.0, index=df.index)
    bp[high] = 1
    bp[missing] = np.nan
    risk = pd.Series(np.nan, index=df.index)
    risk[(med_bp == 1) | (bp == 1)] = 1
    risk[(med_bp == 0) & (bp == 0)] = 0
...
def whr(df, visit):
    ...
    out[(df["X31.0.0"] == 0) & (ratio >= 0.85)] = 1
    out[(df["X31.0.0"] == 1) & (ratio >= 0.9)] = 1
...
def main():
    ...
    df = read_table(Path(TEMPDIR) / "df_visits_cleaned_Apr2021.dat").drop(
        columns=[
            c
            for c in ["X53.2.0", "X53.3.0", "X25000.2.0", "X25000.3.0"]
            if c in read_table(Path(TEMPDIR) / "df_visits_cleaned_Apr2021.dat").columns
        ]
    )
    dfv = df.merge(vis2, on="eid_34077", how="left").merge(
        vis3, on="eid_34077", how="left"
    )
    chol2, bpm2 = medication(dfv, 2)
    chol3, bpm3 = medication(dfv, 3)
    dia2, dia3 = diabetes(dfv, 2), diabetes(dfv, 3)
    dia2[(dia2.isna()) & (dia3 == 0)] = 0
    ...
    dfv["smoking_score_vis2"] = smoking_score(dfv, 2)
    dfv["smoking_score_vis3"] = smoking_score(dfv, 3)
    dfv["medication_chol_vis2"] = chol2
    dfv["medication_chol_vis3"] = chol3
    dfv["BP_risk_vis2"] = bp2
    dfv["BP_risk_vis3"] = bp3
    dfv["diabetes_vis2"] = dia2
    dfv["diabetes_vis3"] = dia3
    dfv["whr_indicator_vis2"] = whr(dfv, 2)
    dfv["whr_indicator_vis3"] = whr(dfv, 3)
    dfv["systolic_vis1"] = sys1
    dfv["systolic_vis2"] = sys2
    dfv = dfv.merge(apoe[["eid_34077", "apoe_score"]], on="eid_34077", how="left")
    v2 = [
        "smoking_score_vis2",
        "medication_chol_vis2",
        "BP_risk_vis2",
        "diabetes_vis2",
        "whr_indicator_vis2",
        "apoe_score",
    ...
    v3 = [
        "smoking_score_vis3",
        "medication_chol_vis3",
        "BP_risk_vis3",
        "diabetes_vis3",
        "whr_indicator_vis3",
        "apoe_score",
    ]
    dfv["CVR_vis2"] = dfv[v2].sum(axis=1, min_count=len(v2))
    dfv["CVR_vis3"] = dfv[v3].sum(axis=1, min_count=len(v3))
    complete_df = dfv.dropna(subset=["CVR_vis2", "CVR_vis3"])
    save_rdata(
        Path(TEMPDIR) / "CVR_9June2021.Rdata", df_visits=dfv, complete_df=complete_df
    )
