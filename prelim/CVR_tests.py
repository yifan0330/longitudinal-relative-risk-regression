#!/usr/bin/env python3
"""Run log-Poisson GEE approximations for selected CVR example voxels."""
...
def pgee_log_poisson(y, x, n_subj, n_visits, max_iter=25):
    ...
    return {
        "beta": res.params,
        "beta_se_sandwich": res.bse,
        "zscore": res.params / res.bse,
        "alpha": getattr(res.cov_struct, "dep_params", None),
    }
...
def prepare_complete_df(cvr=True):
    ...
    if cvr:
        complete_df = (
            complete_df.iloc[:, 1:]
            if complete_df.columns[0] not in ["eid_8107"]
            else complete_df
        )
    ...
    for col in ["age_vis2", "headsize", "systolic_vis1"]:
        out[col] = center(out[col])
    return out
...
def fit_voxel(lesions_vis1, lesions_vis2, complete_df, voxel_idx, cvr=True):
    ...
    y1 = np.asarray(lesions_vis1[voxel_idx]).ravel()
    y2 = np.asarray(lesions_vis2[voxel_idx]).ravel()
    panel = pd.DataFrame(
        {
            "y": np.r_[y1, y2],
            "vis": np.r_[np.ones(n_subj), np.full(n_subj, 2)],
            "eid_8107": np.r_[complete_df["eid_8107"], complete_df["eid_8107"]],
        }
    )
    panel = panel.merge(complete_df, on="eid_8107", how="left").sort_values(
        ["eid_8107", "vis"]
    )
    ...
    return pgee_log_poisson(
        panel["y"].to_numpy(float), x.astype(float), len(panel) // 2, 2
    )


def main():
    obj = load_rdata(Path(TEMPDIR) / "lesions_atleast6_CVR.RData")
    v1, v2 = np.asarray(obj["lesions_vis1"]), np.asarray(obj["lesions_vis2"])
    complete_df = prepare_complete_df(cvr=True)
    models = {}
    for name, voxel_idx in {
        "pgee_model_v1": 6928,
        "pgee_model_v2": 3193,
        "pgee_model_v3": 12621,
        "pgee_model_v4": 6000,
    }.items():
        models[name] = fit_voxel(v1, v2, complete_df, voxel_idx, cvr=True)
        print(name, models[name]["beta"])
    save_rdata(Path(TEMPDIR) / "CVR_temp_July.Rdata", **models)
