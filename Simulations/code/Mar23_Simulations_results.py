...
def summarize_series(
    ...
    *,
    ...
    bias_complete_orpgee = np.zeros((n, 2))
    mse_complete_orpgee = np.zeros((n, 2))
    for j in range(n):
        beta = beta_values[j]
        ...
        nonsep_mask = (
            base.se_ok(ratio_gee, "lt")
            & base.se_ok(ratio_pgee, "lt")
            & base.complete_cases(ratio_gee)
            & base.complete_cases(ratio_pgee)
            & base.complete_cases(coefs_gee)
            & arr(env, "conv_geePK").astype(bool)
        )
        nonsep = base.idx_from_mask(nonsep_mask)
        print(len(nonsep))
        or_idx = np.intersect1d(
            nonsep,
            base.idx_from_mask(
                base.complete_cases(arr(env, "coefs_ORpgee"))
                & (arr(env, "iter_ORpgee") != 25)
            ),
        )
        nonsep = or_idx
        print(len(or_idx))
        ...
        sep = base.idx_from_mask(
            base.se_ok(ratio_gee, "gt")
            & base.complete_cases(ratio_gee)
            & base.complete_cases(ratio_pgee)
        )
        ...
        bias_complete_orpgee[j, :] = (
            np.sum(or_rr[or_idx, :], axis=0) / len(or_idx) - beta[1:3]
        )
        mse_complete_orpgee[j, :] = np.sum(
            (or_rr[or_idx, :] - beta[1:3]) ** 2, axis=0
        ) / len(or_idx)
        print("--unconditional--")
        complete_pgee = (
            base.complete_cases(ratio_pgee)
        ...
        base.show(np.mean(arr(env, "SEs_PgeePK")[complete_pgee, :] ** 2, axis=0))
        print("variance coefs PGEE")
        base.show(np.std(coefs_pgee[complete_pgee, :], axis=0, ddof=1) ** 2)
        base.show(
...
def alpha_followup() -> None:
    ...
    idx = base.idx_from_mask(
        base.se_ok(arr(env, "SEs_model_ratio_geePK"), "lt")
        & base.complete_cases(arr(env, "SEs_model_ratio_geePK"))
        & base.complete_cases(arr(env, "SEs_model_ratio_PgeePK"))
        & arr(env, "conv_geePK").astype(bool)
    )
    print(len(idx))
    base.show(np.mean(arr(env, "SEs_geePK")[idx, :], axis=0))
    base.show(np.std(arr(env, "coefs_geePK")[idx, :], axis=0, ddof=1))
    base.show(
        np.mean(arr(env, "SEs_geePK")[idx, :], axis=0)
        / np.std(arr(env, "coefs_geePK")[idx, :], axis=0, ddof=1)
    )
    base.show(np.mean(arr(env, "SEs_PgeePK")[idx, :], axis=0))
    base.show(np.std(arr(env, "coefs_PgeePK")[idx, :], axis=0, ddof=1))
    base.show(
...
def zsep_plots() -> None:
    ...
    try:
        ...
        complete = base.idx_from_mask(
            base.se_ok(ratio_gee, "le")
            & base.complete_cases(ratio_gee)
            & base.complete_cases(ratio_pgee)
            & arr(env, "conv_geePK").astype(bool)
        )
        ...
        sep_null = base.idx_from_mask(
            base.se_ok(ratio_gee, "lt")
            & base.complete_cases(ratio_gee)
            & ~arr(env, "conv_geePK").astype(bool)
        )
        print(len(sep_null))
        base.show(coefs_gee[sep_null, :])
        base.show(arr(env, "SEs_model_geePK")[sep_null, :])
        base.show(coefs_pgee[sep_null, :])
        if "tabdat" in env:
            base.show(np.asarray(env["tabdat"], dtype=object)[sep_null])
        print(
            np.mean(
                np.abs(coefs_gee[complete, 1] - 1.6) / ses_gee[complete, 1]
                >= base.stats.t.ppf(0.975, 47)
            )
        )
        print(
            np.mean(
        ...
        g = base.sns.displot(data=se_ratio, x="x", col="group", bins=30)
...
def main() -> int:
    use_mar23_dir()
    base.diagnostic_counts("test_alpha", 7)
    or_diagnostics("test_alpha", 7)
    alpha_results = summarize_series(
        "test_alpha", 7, [BETA.copy() for _ in range(7)], ALPHAS, "alpha"
    )
    base.show(alpha_results[0])
    base.show(alpha_results[1])
    base.show(alpha_results[2][:, 1])
    base.show(alpha_results[3][:, 1])
    base.show(alpha_results[4][:, 1])
    base.show(alpha_results[6][:, 1])
    base.show(alpha_results[12][:, 0])
    base.show(alpha_results[5][:, 1])
    base.show(alpha_results[7][:, 1])
    base.show(alpha_results[13][:, 0])
    alpha_followup()
    zsep_plots()
    base.diagnostic_counts("BetaB_", 5)
    beta_b_results = base.summarize_series(
        "BetaB_", 5, [np.array([-4.0, b, 0.2]) for b in BETA_B], BETA_B, "beta"
    )
    base.show(beta_b_results[8][:, 1])
    base.show(beta_b_results[10][:, 1])
    base.show(beta_b_results[9][:, 1])
    base.show(beta_b_results[11][:, 1])
    base.show(beta_b_results[4][:, 1])
    base.show(beta_b_results[6][:, 1])
    base.show(beta_b_results[5][:, 1])
    base.show(beta_b_results[7][:, 1])
    base.show(beta_b_results[2][:, 1])
    base.show(beta_b_results[3][:, 1])
    base.diagnostic_counts("test_N_", 6)
    or_diagnostics("test_N_", 6)
    n_results = summarize_series("test_N_", 6, [BETA.copy() for _ in range(6)], NS, "N")
    base.show(n_results[0])
    base.show(n_results[1])
    base.show(n_results[4][:, 1])
    base.show(n_results[6][:, 1])
    base.show(n_results[12][:, 0])
    base.show(n_results[5][:, 1])
    base.show(n_results[7][:, 1])
    base.show(n_results[13][:, 0])
    base.show(n_results[2][:, 1])
    base.show(n_results[3][:, 1])
    base.diagnostic_counts("test_Beta0", 3)
    or_diagnostics("test_Beta0", 3)
    ...
    base.show(beta0_results[0])
    base.show(beta0_results[1])
    base.show(beta0_results[4][:, 1])
    base.show(beta0_results[6][:, 1])
    base.show(beta0_results[12][:, 0])
    base.show(beta0_results[5][:, 1])
    base.show(beta0_results[7][:, 1])
    base.show(beta0_results[13][:, 0])
    base.show(beta0_results[2][:, 1])
    base.show(beta0_results[3][:, 1])
    base.diagnostic_counts("gamma", 7)
    gamma_results = base.summarize_series(
        "gamma",
        7,
    ...
    base.show(gamma_results[0])
    base.show(gamma_results[1])
    base.show(gamma_results[4][:, 1])
    base.show(gamma_results[6][:, 1])
    base.show(gamma_results[5][:, 1])
    base.show(gamma_results[7][:, 1])
    base.show(gamma_results[2][:, 1])
    base.show(gamma_results[3][:, 1])
    return 0
