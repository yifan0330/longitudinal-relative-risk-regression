...
def main(argv: list[str] | None = None) -> int:
    ...
    coefs_geePK = np.zeros((n_sim, p))
    SEs_geePK = np.zeros((n_sim, p))
    SEs_model_geePK = np.zeros((n_sim, p))
    SEs_model_ratio_geePK = np.zeros((n_sim, p))
    ...
    coefs_PgeePK = np.zeros((n_sim, p))
    SEs_PgeePK = np.zeros((n_sim, p))
    ...
    for i in range(1, n_sim + 1):
        ...
        datt = gen_dataPP(
            beta=true_beta, nc=nc, cl_size=cl_size, p=prop, rho=rho, rng=rng
        )
        ...
        if gee_PK is None:
            ...
            alpha_geePK[i - 1] = np.nan
            phi_geePK[i - 1] = np.nan
            eta_geePK[i - 1] = np.nan
            iter_geePK[i - 1] = np.nan
            conv_geePK[i - 1] = False
        else:
        ...
        if Pgee_PK is None:
            ...
            alpha_PgeePK[i - 1] = np.nan
            phi_PgeePK[i - 1] = np.nan
            eta_PgeePK[i - 1] = np.nan
            iter_PgeePK[i - 1] = np.nan
            conv_PgeePK[i - 1] = False
        else:
