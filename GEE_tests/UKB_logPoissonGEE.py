...
def describe(x):
    return pd.Series(np.asarray(x, dtype=float).reshape(-1)).describe(percentiles=[0.25, 0.5, 0.75])


def map_summary(result_dir: Path, prefix: str, cov: str, voxel_ids: np.ndarray, quantiles=(0, 0.25, 0.5, 0.75, 0.99, 1)):
...
def se_trace_last(output_all, key="beta_se_model_trace", n_cov=4, expected_len=(6, 8, 9)) -> np.ndarray:
    cols = []
...
def fit_statsmodels_poisson(panel: pd.DataFrame):
    ...
    groups = np.repeat(np.arange(len(panel) // panel["vis"].nunique()), panel["vis"].nunique())
...
def plot_scatter(x, y, title, out_path, xlim=(0, 10), ylim=(0, 10), xlabel=None, ylabel=None):
...
def inspect_voxel(lesions_vis1, lesions_vis2, idx, ids, df_visits, mode="original"):
    ...
    X = design_matrix(panel)
    if mode == "penalty":
        return gee_penalty_run(panel["y"].to_numpy(), X, n_subj, n_visits, covariance="Exchangeable")
    if mode == "dispersion":
        return gee_dispersion_run(panel["y"].to_numpy(), X, n_subj, n_visits, covariance="Exchangeable")
    return gee_run(panel["y"].to_numpy(), X, n_subj, n_visits, covariance="Exchangeable")


def separation_flags(lesions_vis1: np.ndarray, lesions_vis2: np.ndarray, ids: np.ndarray, df_visits: pd.DataFrame):
    ...
    for i in range(lesions_vis1.shape[0]):
        ...
        s = np.r_[sex, sex]
        v = np.r_[np.ones(lesions_vis1.shape[1], int), np.full(lesions_vis1.shape[1], 2, int)]
        f_les = np.sum((y == 1) & (s == 0)); m_les = np.sum((y == 1) & (s == 1))
        v1_les = np.sum((y == 1) & (v == 1)); v2_les = np.sum((y == 1) & (v == 2))
...
def main() -> int:
    ...
    for result in ["results_poisson_sexM_independent", "results_poisson_sexM_independent_geePK", "results_poisson_sexM_exch_geePK"]:
    ...
    se_intercept = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "se_Intercept_GEE")
    intercept_potential = np.where(flat_values(se_intercept, voxel_ids) > 6.92)[0]
    print(len(intercept_potential))
    output_all = load_output(GEEDIR / "results_poisson_sexM_exch_geePK" / "results_exch_GEE.Rdata").get("output_all", [])
    ...
    print(summary_PK(flat_values(empir_prob_vis1, voxel_ids[NA_ids]), [0, 0.25, 0.5, 0.75, 0.99, 1]))
    print(summary_PK(flat_values(empir_prob_vis2, voxel_ids[NA_ids]), [0, 0.25, 0.5, 0.75, 0.99, 1]))

    ids = load_ids(); df_visits = load_visits(); lesions_vis1, lesions_vis2 = load_lesions()
    if len(NA_ids):
        model = inspect_voxel(lesions_vis1[NA_ids], lesions_vis2[NA_ids], 0, ids, df_visits)
        print(model["beta"], model["beta_se_sandwich"], model["alpha"])
        model_phi = inspect_voxel(lesions_vis1[NA_ids], lesions_vis2[NA_ids], 0, ids, df_visits, mode="dispersion")
        print(model_phi["beta"], model_phi["beta_se_sandwich"], model_phi["alpha"], 1 / model_phi["phi"])

    age_map = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "se_avg_age_GEE")
    time_map = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "se_age_diff_GEE")
    NA_ids_time = np.where(np.isnan(flat_values(time_map, voxel_ids)) & ~np.isnan(flat_values(age_map, voxel_ids)))[0]
    time_est = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "estimate_age_diff_GEE")
    sexM_est = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "estimate_sexM_GEE")
    print(flat_values(time_est, voxel_ids[NA_ids_time]))
    print(flat_values(sexM_est, voxel_ids[NA_ids_time]))
    if len(NA_ids_time):
        same = np.sum(np.all(lesions_vis1[NA_ids_time] == lesions_vis2[NA_ids_time], axis=1))
        print(same)
        inspect_voxel(lesions_vis1[NA_ids_time], lesions_vis2[NA_ids_time], min(1, len(NA_ids_time) - 1), ids, df_visits)

    alpha_map2 = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK" / "alpha_GEE")
    NA_ids_age = np.where(np.isnan(flat_values(age_map, voxel_ids)) & ~np.isnan(flat_values(alpha_map2, voxel_ids)))[0]
    ...
    if len(intersect_SEcheck):
        gee_model = inspect_voxel(lesions_vis1[intersect_SEcheck], lesions_vis2[intersect_SEcheck], min(3, len(intersect_SEcheck) - 1), ids, df_visits)
        print(gee_model["beta_se_model"], gee_model["beta_se_sandwich"])

    sex_separated, visit_separated = separation_flags(lesions_vis1, lesions_vis2, ids, df_visits)
    print(int(np.sum(sex_separated)), int(np.sum(visit_separated)), int(np.sum(sex_separated & visit_separated)))
    print(np.intersect1d(np.where(sex_separated)[0], idx_SEratio))

    output_all_max25 = load_output(GEEDIR / "results_poisson_sexM_exch_geePK_max25" / "results_exch_GEE.Rdata").get("output_all", [])
    last_model = se_trace_last(output_all_max25, key="beta_se_model_trace", expected_len=(8, 9))
    for j in range(4):
        print(summary_PK(last_model[j], [0, 0.01, 0.25, 0.5, 0.75, 0.99, 1]))
    idx_max25 = np.where(np.isnan(last_model[0]))[0]
    if len(intercept_potential):
        penalty_model = inspect_voxel(lesions_vis1[intercept_potential], lesions_vis2[intercept_potential], 0, ids, df_visits, mode="penalty")
        orig_model = inspect_voxel(lesions_vis1[intercept_potential], lesions_vis2[intercept_potential], 0, ids, df_visits)
    ...
    est_map = read_nifti(GEEDIR / "results_poisson_sexM_exch_geePK_max25" / "estimate_sexM_GEE")
    dvrg_idx = sex_separated
    print(summary_PK(flat_values(est_map, voxel_ids)[dvrg_idx], [0, 0.25, 0.5, 0.75, 0.99, 1]))
    pgee_est = read_nifti(GEEDIR / "results_penalty_poisson_sexM_exch_geePK_max25" / "estimate_sexM_GEE")
    print(summary_PK(flat_values(pgee_est, voxel_ids)[dvrg_idx], [0, 0.25, 0.5, 0.75, 0.99, 1]))
