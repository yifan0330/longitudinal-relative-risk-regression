...
def main() -> int:
    ...
    _, se_trace_last_iter = se_trace_last(
        output_all, key="beta_se_model_trace", expected_len=(8, 9)
    )
    for idx, name in enumerate(NAMES_COVS):
        print(
            name,
            summary_PK(se_trace_last_iter[idx], [0, 0.01, 0.25, 0.5, 0.75, 0.99, 1]),
        )
    plot_hist(
        se_trace_last_iter[3],
        "SE ratio: Sex",
        GEEDIR / "figures" / "dispersion_se_ratio_sex.png",
    )
    ...
    print(
        summary_PK(
            flat_values(sexM_est, voxel_ids[keep]), [0, 0.25, 0.5, 0.75, 0.99, 1]
        )
    )
    ...
    if len(NA_ids):
        ...
        panel = make_panel_from_arrays(
            min(9, len(NA_ids) - 1), subset_lesions1, subset_lesions2, ids, df_visits
        )
        ...
        print(gee_model)
        gee_model_phi = gee_dispersion_run(
            panel["y"].to_numpy(), X, n_subj, n_visits, covariance="Exchangeable"
        )
    ...
    if dispersed_file.exists():
        ...
        output_dispersed = []
    if output_dispersed:
        _, last_d = se_trace_last(
            output_dispersed, key="beta_se_model_trace", expected_len=(8, 9)
        )
        print(summary_PK(last_d[3], [0, 0.01, 0.25, 0.5, 0.75, 0.95, 0.99, 1]))
        print(int(np.sum(last_d[3] > 100)), int(np.sum(last_d[3] > 10)))
        alpha = [
            item.get("alpha", np.nan) if isinstance(item, dict) else np.nan
            for item in output_dispersed
        ]
    ...
    if high_file.exists():
        ...
        output_high = []
    if output_high:
        traces_h, _ = se_trace_last(
            output_high, key="beta_se_model_trace", expected_len=(8, 9)
        )
    ...
    alpha_penalized = read_nifti(
        GEEDIR / "results_penalty_poisson_sexM_exch_geePK_max25" / "alpha_GEE.nii.gz"
    )
    alpha_original = read_nifti(
        GEEDIR / "results_poisson_sexM_exch_geePK_max25" / "alpha_GEE.nii.gz"
    )
    ...
    plt.savefig(
        GEEDIR / "figures" / "alpha_original_vs_penalized.png", bbox_inches="tight"
    )
    plt.close()
