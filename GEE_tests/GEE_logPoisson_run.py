...
def main() -> int:
    ...
    def fit_one(i: int):
        try:
            ...
            gee_model = gee_run(y=panel["y"].to_numpy(), X=X, n_subj=n_subj, n_visits=n_visits, covariance="Exchangeable", tol=1e-4, max_iter=25)
            if i % 100 == 99:
                print(i + 1)
            out = {
    ...
    save_output(output, GEEDIR / "temp_poisson_sexM_exch_geePK_max25" / f"GEE_subset_{args.j}.RData")
    return 0
