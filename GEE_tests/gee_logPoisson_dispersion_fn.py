...
def gee_dispersion_run(
    ...
    for _ in range(max_iter):
        ...
        cov = covariance.lower()
        if cov == "independence":
            alpha, R = 0.0, np.eye(n_visits)
        ...
        tmp4 = tmp1 @ _block_diag(_safe_inv(R), n_subj) @ tmp1 * phi
        tmp5 = tmp2.T @ tmp4
        I = tmp5 @ tmp2
        U = tmp5 @ (y - mu)
        I_inv = _spdinv(I)
        ...
        cov_mat = I_inv @ temp_v3 @ I_inv
        se_sandwich = np.sqrt(np.clip(np.diag(cov_mat), 0, None))
