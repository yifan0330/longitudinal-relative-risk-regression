...
def write_img(data: np.ndarray, template: nib.Nifti1Image, stem: str | Path) -> None:
    ...
    out = (
        stem if str(stem).endswith((".nii", ".nii.gz")) else Path(str(stem) + ".nii.gz")
    )
    nib.save(
        nib.Nifti1Image(np.asarray(data), template.affine, template.header), str(out)
    )
...
def r_linear_set(
    data: np.ndarray, ids_1based: np.ndarray, values: np.ndarray
) -> np.ndarray:
...
def finite_summary(name: str, values: np.ndarray) -> None:
    ...
    if vals.size:
        print(
            f"{name}: min={vals.min():.4g} median={np.median(vals):.4g} mean={vals.mean():.4g} max={vals.max():.4g}"
        )
    else:
...
def plot_slice(
    ...
    im = plt.imshow(
        overlay, cmap=cmap, vmin=vmin, vmax=vmax, alpha=0.78, origin="lower"
    )
...
def scatter_pdf(
    x: np.ndarray, y: np.ndarray, out: Path, xlabel: str, ylabel: str, limit: float
) -> None:
    out.parent.mkdir(parents=True, exist_ok=True)
    ...
    if len(df):
        sns.kdeplot(
            data=df, x="x", y="y", fill=True, levels=20, cmap="viridis", thresh=0.02
        )
        sns.scatterplot(
...
def main() -> None:
    ...
    estimates, stderror, alpha, phi, iterations, output_all = collect_results(
        len(voxel_ids)
    )
    zscores = estimates / stderror
    image_zero = brain_mask.copy()
    image_zero[image_zero != 0] = 0
    ...
    write_img(
        r_linear_set(image_zero.copy(), voxel_ids, alpha),
        mask_template,
        RESULT_DIR / "alpha_GEE",
    )
    write_img(
        r_linear_set(image_zero.copy(), voxel_ids, phi),
        mask_template,
        RESULT_DIR / "phi_GEE",
    )
    write_img(
        r_linear_set(image_zero.copy(), voxel_ids, iterations),
        mask_template,
        RESULT_DIR / "iterations_GEE",
    )
    save_rdata(
        RESULT_DIR / "results_CVR_PGEE_1578subjs.Rdata",
        estimates=estimates,
        stderror=stderror,
        zscores=zscores,
        alpha=alpha,
    ...
    for i, cov in enumerate(NAMES_COVS):
        ...
        finite_summary(
            f"|z|>1.96 {cov}",
            np.array([np.sum(np.abs(r_linear_get(z_img, voxel_ids)) > 1.96)]),
        )
        plot_slice(
            45,
            z_img,
    ...
    for sl, leg in ((40, False), (45, False), (50, True)):
        plot_slice(
            ...
            mni152,
            voxel_ids,
            GEEDIR / "plots" / "empirical" / "empir_vis1_z",
            legend=leg,
            vmin=0,
            vmax=0.7,
            cmap="viridis",
    ...
    for sl, leg in ((40, False), (45, False), (50, True)):
        plot_slice(
            ...
            mni152,
            voxel_ids,
            GEEDIR / "plots" / "empirical" / "empir_RR_z",
            legend=leg,
            vmin=0,
            vmax=2,
            cmap="RdBu_r",
    ...
    for subdir, prefix, result_name in (
        ("gee", "", "results_July_gee_interaction"),
        ("pgee", "Sept_", "results_Sept_pgee_interaction"),
    ):
            ...
            if path.exists():
                ...
                for sl, leg in ((40, False), (45, False), (50, True)):
                    plot_slice(
                        ...
                        mni152,
                        voxel_ids,
                        GEEDIR / "plots" / subdir / f"{prefix}{param}_z",
                        title=param if sl == 40 else "",
                        legend=leg,
                        vmin=bounds[0],
                        vmax=bounds[1],
                        cmap="RdBu_r",
    ...
    rr_specs = [
        ("baseAge", "rr_age_z", "Age (visit 1)", 2),
        ("ageDiff", "rr_ageDiff_z", "Time difference", 2),
        ("baseCVR", "rr_CVR_z", "CVR (visit 1)", 3),
        ("CVRdiff", "rr_CVRDiff_z", "CVR diff.", 3),
    ]
    rr_images = {}
    for cov, plotname, title, vmax in rr_specs:
        beta, _ = read_img(RESULT_DIR / f"estimate_{cov}_GEE.nii.gz")
        rr = np.exp(beta)
        rr.ravel(order="F")[np.setdiff1d(np.arange(rr.size), voxel_ids - 1)] = np.nan
        rr_images[cov] = rr
        ...
        for sl, leg in ((40, False), (45, False), (50, True)):
            plot_slice(
                ...
                mni152,
                voxel_ids,
                GEEDIR / "plots" / "pgee" / "rr" / plotname,
                title=title if sl == 40 else "",
                legend=leg,
                vmin=0,
                vmax=vmax,
                cmap="RdBu_r",
    ...
    groups = [
        np.where(emp_vals < 0.0025)[0],
        np.where((emp_vals < 0.005) & (emp_vals >= 0.0025))[0],
        np.where((emp_vals < 0.01) & (emp_vals >= 0.005))[0],
        np.where(emp_vals >= 0.01)[0],
    ]
    age_x = r_linear_get(rr_images["baseAge"], voxel_ids)
    age_y = r_linear_get(rr_images["ageDiff"], voxel_ids)
    cvr_x = r_linear_get(rr_images["baseCVR"], voxel_ids)
    cvr_y = r_linear_get(rr_images["CVRdiff"], voxel_ids)
    scatter_pdf(
        age_x,
        age_y,
        GEEDIR / "plots" / "pgee" / "rr" / "rr_age_ageDiff_scatter.pdf",
        "PGEE Age (visit 1)",
        "PGEE Time difference",
        2,
    )
    scatter_pdf(
        cvr_x,
        cvr_y,
        GEEDIR / "plots" / "pgee" / "rr" / "rr_CVR_CVRDiff_scatter.pdf",
        "PGEE CVR (visit 1)",
        "PGEE CVR difference",
        3,
    )
    for idx, grp in enumerate(groups, start=1):
        scatter_pdf(
            age_x[grp],
            age_y[grp],
            GEEDIR / "plots" / "pgee" / "rr" / f"rr_age_ageDiff_scatter_empir{idx}.pdf",
            "PGEE Age (visit 1)",
            "PGEE Time difference",
            2,
        )
        scatter_pdf(
            cvr_x[grp],
            cvr_y[grp],
            GEEDIR / "plots" / "pgee" / "rr" / f"rr_CVR_CVRDiff_scatter_empir{idx}.pdf",
            "PGEE CVR (visit 1)",
            "PGEE CVR difference",
            3,
        )
