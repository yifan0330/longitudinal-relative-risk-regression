...
```bash
uv run python -m RR_OR_GEE_PGEE.run_simulation --mode smoke
```

This uses the same scenario names as the full run, but with 100 replications per scenario. Expected outputs under `RR_OR_GEE_PGEE/results/smoke/`:
...
```bash
uv run python -m RR_OR_GEE_PGEE.plot_figure1
```
...
`RR_OR_GEE_PGEE/results/full/figure1/`. Each plot uses the same data-driven
...
```bash
uv run python -m RR_OR_GEE_PGEE.plot_figure1 --boundary-warnings --stem figure1_rr_zscore_scatter_boundary
```
...
```bash
uv run python -m RR_OR_GEE_PGEE.plot_figure1 --metric beta
uv run python -m RR_OR_GEE_PGEE.plot_figure1 --metric se
```

To generate the same three subfigures for OR-GEE vs OR-PGEE, run:

```bash
uv run python -m RR_OR_GEE_PGEE.plot_figure1 --pair or --metric all
```
...
```bash
uv run python -m RR_OR_GEE_PGEE.plot_pp
```
...
`RR_OR_GEE_PGEE/results/full/pp_plots/`. Every method uses the same subset
...
`RR_OR_GEE_PGEE/results/full/table1/`, `table2/`, `table3/`, `table4/`,
...
```bash
uv run python -m RR_OR_GEE_PGEE.run_simulation --mode full --scenario base
```

To run all configured scenarios in one process:

```bash
uv run python -m RR_OR_GEE_PGEE.run_simulation --mode full
```

To parallelize over independent simulation replications within a scenario:

```bash
uv run python -m RR_OR_GEE_PGEE.run_simulation --mode full --scenario base --workers 8
```
...
```bash
uv run python -m RR_OR_GEE_PGEE.benchmark_speed --replications 20 --workers 8 --include-lbfgs
```

This writes `RR_OR_GEE_PGEE/results/benchmark/speed_comparison.csv`.
...
```bash
uv run python -m RR_OR_GEE_PGEE.run_simulation --mode full --confounding-shift 1.00 --scenario confounded_shift_1_00
```
...
`coverage.py::coverage_from_saved_payload` computes RR-GEE/RR-PGEE coverage from saved per-replication arrays (`coefs_*`, `SEs_*`, `conv_*`) using the sandwich SEs. Use it for existing published-result payloads when they are present; do not rerun RR-GEE/RR-PGEE for those shared scenarios solely to compute coverage. In the current checked-out tree no `Simulations/**/*.RData`, `.Rdata`, `.pkl`, or `.pickle` payloads were present, so the smoke test generated fresh paired outputs only under `RR_OR_GEE_PGEE/results/smoke/`.
