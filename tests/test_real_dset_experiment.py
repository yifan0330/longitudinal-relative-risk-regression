...
from real_dset_experiment.cli import (
    FIGURE_COMMANDS,
    TABLE_COMMANDS,
...
class PathAndModelTests(unittest.TestCase):
    ...
    def test_model_result_paths_normalize_hyphens(self) -> None:
        paths = ExperimentPaths.from_package_dir(Path("/tmp/project/real_dset_experiment"))
        self.assertEqual(
            paths.model_result_dir("rr-pgee"),
            Path("/tmp/project/real_dset_experiment/UKB/python_results/rr_pgee"),
        )
